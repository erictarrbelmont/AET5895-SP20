...
float PitchShifter::processSample(float x, int channel){
    ...
    float x1 = pitchDelay1.processSample(x, channel);
    float x2 = pitchDelay2.processSample(x, channel);
    float x3 = pitchDelay3.processSample(x, channel);
...
void PitchShifter::setFs(float Fs){
    ...
    float period = (MAX_DELAY_SAMPLES-2.f) / (delta*Fs);
    freq = 1.f/period;
...
void PitchShifter::setPitch(float semitone){
    ...
    angleChange = fabs(freq * (2.f*M_PI) / Fs);
